...
class LeadIntakeService
{
    ...
    {
        return \Illuminate\Support\Facades\DB::transaction(function () use ($data, $companyId, $actorUserId) {
            ...
            if (!empty($data['meta'])) {
                $existingMeta = is_array($lead->meta) ? $lead->meta : (json_decode($lead->meta ?? '{}', true) ?: []);
                $lead->meta = array_merge($existingMeta, $data['meta']);
                $lead->save();
            }
